...
JOB_NAME=${TRAVIS_JOB_NAME:-Mageia cauldron}

arr=($JOB_NAME)
os_name=${arr[0]:-Mageia}
release=${arr[1]:-cauldron}
...
sed -e "s/@IMAGE@/docker.io\/library\/$os:$release/" \
...
docker run -e COVERITY_SCAN_TOKEN=$COVERITY_SCAN_TOKEN -e TRAVIS=$TRAVIS -e TRAVIS_JOB_NAME="$TRAVIS_JOB_NAME" --rm fedora-modularity/libmodulemd:$release
